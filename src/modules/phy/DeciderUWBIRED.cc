...
simtime_t DeciderUWBIRED::processSignalHeader(AirFrame* frame) {
...
simtime_t DeciderUWBIRED::getNextSignalHandleTime(const AirFrame* frame) const {
...
bool DeciderUWBIRED::attemptSync(const AirFrame* frame) {
...
DeciderResult* DeciderUWBIRED::createResult(const AirFrame* frame) const {
 ...
 */
std::pair<bool, double> DeciderUWBIRED::decodePacket(const AirFrame* frame, vector<bool> * receivedBits, const IEEE802154A::config& cfg) const {
